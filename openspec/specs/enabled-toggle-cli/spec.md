...
The persistent global on/off surface: `omnictx on` / `omnictx off` write the `enabled:` key to the config file so all future shells see it, complementing the session-scoped `OMNICTX_ENABLED` env var. The surface is deliberately minimal — `toggle`, `enable`, and `disable` are not subcommands (removed by `remove-toggle-leftovers`).
