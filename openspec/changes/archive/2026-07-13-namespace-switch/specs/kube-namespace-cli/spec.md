...
form that enumerates cluster namespaces. There is no reserved `list` argument:
`omnictx ns list` SHALL be treated as a request to set the namespace to
the literal value `list` (a valid DNS-1123 label), subject to the same
validation and write rules as any other name.

#### Scenario: `list` is a namespace name, not a listing form
- **WHEN** the active context is resolvable and the user runs `omnictx ns list`
- **THEN** the active context's namespace is set to `list` and the exit code is 0
